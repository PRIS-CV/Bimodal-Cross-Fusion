...
class DeformableDetrTransformerEncoderLayer(DetrTransformerEncoderLayer):
    ...
    def _init_layers(self) -> None:
        ...
        self.self_attn = MultiScaleDeformableAttentionV2(**self.self_attn_cfg)
...
class DeformableDetrTransformerDecoderLayer(DetrTransformerDecoderLayer):
    ...
    def _init_layers(self) -> None:
        ...
        self.cross_attn = MultiScaleDeformableAttentionV2(**self.cross_attn_cfg)
        self.embed_dims = self.self_attn.embed_dims
        self.ffn = FFN(**self.ffn_cfg)
        norms_list = [
            build_norm_layer(self.norm_cfg, self.embed_dims)[1]
...
class MultiScaleDeformableAttentionV2(BaseModule):
    ...
    def __init__(self,
        ...
        self.attention_weights = MSCBAM(embed_dims,num_heads*num_levels*num_points)
        self.Linear = nn.Linear(embed_dims,num_heads*num_levels*num_points)
    ...
    def forward(self,
        ...
        # TODO 此处需做特殊处理，在最后一步的特征无法与spatial_shapes对应上
        # 该情况仍使用linear
        spatial_sum = 0
        for (h,w) in spatial_shapes:
            spatial_sum += h*w
        if num_query != spatial_sum:
            attention_weights = self.Linear(query).view(
                bs, num_query, self.num_heads, self.num_levels * self.num_points)
        else:
            attention_weights = self.attention_weights(query,spatial_shapes,level_start_index).view(
                bs, num_query, self.num_heads, self.num_levels * self.num_points)
...
# 另行定义一个模块，取代nn.Linear(embed_dims, num_heads * num_levels * num_points * 2)
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        # [bs,dim,h,w]
        avg_out = self.fc2(self.relu1(self.fc1(self.avg_pool(x))))  # [bs,dim,1,1]
        max_out = self.fc2(self.relu1(self.fc1(self.max_pool(x))))  # [bs,dim,1,1]
        out = avg_out + max_out
...
class MSCBAM(nn.Module):
    def __init__(self, dim1, dim2):
        ...
        self.dim2 = dim2
        # 初始化 CBAM 模块
        self.cbam = CBAM(in_planes=dim1, ratio=16, kernel_size=7)
        # 线性层用于特征融合
        self.linear = nn.Linear(dim1, dim2)
    
    def forward(self, x, spatial_shapes, level_start_index):
        """
        x: 输入特征，形状为 [bs, total_length, dim1]
        spatial_shapes: 每一层的形状，形状为 [num_levels, 2]，表示 (h, w)
        level_start_index: 每一层的起始索引，形状为 [num_levels]
        """
        bs,total_length,_ = x.shape
        num_levels = len(spatial_shapes)
        # 拆分特征到不同层级
        features = []
        for i in range(num_levels):
            start_idx = level_start_index[i]
            if i == num_levels - 1:
            ...
            level_length = h * w
            level_features = x[:, start_idx:end_idx, :]
            level_features = level_features.view(bs, h, w, self.dim1)
            level_features = level_features.permute(0, 3, 1, 2)  # 转换为 [bs, dim1, h, w]
            features.append(level_features)
            
        # 对每一层特征应用 CBAM
        enhanced_features = []
        for f in features:
            enhanced_features.append(self.cbam(f))
        # 将增强后的特征重新拼接
        enhanced_features = [f.permute(0, 2, 3, 1).contiguous().view(bs, -1, self.dim1) for f in enhanced_features]
        enhanced_features = torch.cat(enhanced_features, dim=1)
        # 经过线性层转换
        output = self.linear(enhanced_features)
        return output
